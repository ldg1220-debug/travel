...
export const WORLD_CITIES: WorldCity[] = [
  ...
  // "호치민"은 CITY_SEEDS(위쪽, o-vn-hcm)에 이미 있었지만, 그 배치가
  // pushGeneratedBatch→generateSpots를 거쳐 전부 UNVERIFIED_GENERATED_SPOT_IDS로
  // 필터링돼(이 파일 하단 참고) 실제로는 스팟이 하나도 안 남아
  // OVERSEAS_LOCALITY_NAMES에 못 들어가고 있었다 — REGION_ALIASES가
  // "호찌민"의 정본으로 가리키는 이름인데 정작 그 이름 자체가
  // 미지원이면 별칭이 무의미해진다. 좌표는 위 CITY_SEEDS 항목과 동일.
  ["베트남", "호치민", 10.7769, 106.7009],
  // 아시아 · 태국
  ["태국", "파타야", 12.9236, 100.8825], ["태국", "아유타야", 14.3532, 100.5689], ["태국", "코사무이", 9.512, 100.0136],
  ["태국", "끄라비", 8.0863, 98.9063], ["태국", "후아힌", 12.5684, 99.9577],
  // 태국 보강(신규 — 치앙마이) — 작업지시서 2026-09-23 "한국인이 가장
  // 많이 가는 나라 셋이 0개입니다" §4 6순위.
  ["태국", "치앙마이", 18.7883, 98.9853],
  // 아시아 · 대만
  ...
  // 아시아 · 필리핀 (신규 국가) — 작업지시서 2026-09-23 "한국인이 가장
  // 많이 가는 나라 셋이 0개입니다" — 2026년 한국인 출국 6위(연 100만
  // 명)인데 지원 지역이 0곳이었다. §4 1순위.
  ["필리핀", "세부", 10.3157, 123.8854], ["필리핀", "보라카이", 11.9674, 121.9248],
  ["필리핀", "마닐라", 14.5995, 120.9842], ["필리핀", "팔라완", 9.8349, 118.7384],
  // 작업지시서 2026-09-23 "#267 검증" §3 — 필리핀 수요 2위권인데 목록에
  // 없었다. 좌표는 보홀의 관문 도시 타그빌라란(Tagbilaran City) 기준.
  ["필리핀", "보홀", 9.6474, 123.8558],
  // 아시아 · 인도네시아 (신규 국가) — 출국 10위(연 60만 명). §4 2순위.
  // "발리"는 과거 사고 지역(9월 초 실측: region=발리가 울산 온양읍
  // 스팟을 잘못 반환)이라, 발리 자체가 아니라 그 안의 구체적인 동네
  // (우붓·쿠타·스미냑)를 도시 단위로 넣는다 — 이름이 더 구체적일수록
  // 국내 동명 지역과 오매칭될 확률이 낮다.
  ["인도네시아", "우붓", -8.5069, 115.2625], ["인도네시아", "쿠타", -8.7183, 115.1686],
  ["인도네시아", "스미냑", -8.6913, 115.1594], ["인도네시아", "자카르타", -6.2088, 106.8456],
  // 아시아 · 싱가포르 (신규 국가) — 출국 9위(연 65만 명). §4 3순위.
  // 도시국가라 국가명과 도시명이 같다.
  ["싱가포르", "싱가포르", 1.3521, 103.8198],
  // 아시아 · 말레이시아 (신규 국가) — 이 지시서의 발단(동근님이 런처에
  // "코타키나발루"를 넣었다가 매칭 실패)이 된 나라. §4 4순위.
  ["말레이시아", "코타키나발루", 5.9804, 116.0735], ["말레이시아", "쿠알라룸푸르", 3.139, 101.6869],
  ...
  // 미국령(신규) — 홍콩/마카오가 "중국" 아래 도시로 들어가 있는 것과
  // 같은 방식으로, 괌·사이판도 소속 국가(미국) 아래 도시로 넣는다.
  // 작업지시서 2026-09-23 §4 5순위.
  ["미국", "괌", 13.4443, 144.7937], ["미국", "사이판", 15.1783, 145.7505],
  // 미주 · 캐나다
...
export interface FlatRegion {
  name: string;
  parent: string;
  /** 이 지역을 가리키는 다른 표기(예: "냐짱"의 "나트랑") — REGION_ALIASES의 역방향. 없으면 생략(빈 배열 대신 필드 자체가 없음). */
  aliases?: string[];
...
export function flatRegions(scope: DiscoverScope): FlatRegion[] {
  ...
  if (scope === "overseas") {
    return tree.flatMap((continent) =>
      continent.children.flatMap((country) =>
        country.children.map((city) => attachRegionMeta({ name: city.label, parent: country.label })),
      ),
    );
  }
...
/**
 * 사용자가 쓰는 표기와 공식(카탈로그에 실제 데이터가 있는) 표기가 다른
 * 경우 — 작업지시서 2026-09-23 "한국인이 가장 많이 가는 나라 셋이
 * 0개입니다" §5. 값(오른쪽)이 실제로 카탈로그/WORLD_CITIES에 있는
 * 이름이어야 한다 — alias만 있고 실체가 없으면 이 매핑 자체가 새로운
 * 미지원 지역을 만드는 셈이라 의미가 없다.
 *
 * 지시서는 "호치민 → 호찌민"을 요청했지만, 실제로는 반대다 — 이
 * 카탈로그의 실측 큐레이션 스팟(WORLD_CITIES 근처 참고, "베트남 ·
 * 호치민")이 이미 "호치민"으로 저장돼 있고 "호찌민"은 이 파일 어디에도
 * 없다. 지시서 예시를 그대로 따르면 이미 동작하던 "호치민"을 존재하지
 * 않는 이름으로 리다이렉트해 오히려 깨뜨리게 되므로, 방향을 뒤집었다.
 */
export const REGION_ALIASES: Readonly<Record<string, string>> = {
  나트랑: "냐짱",
  호찌민: "호치민",
  타이페이: "타이베이",
  // 작업지시서 2026-09-23 "#267 검증" §3 — 한국인 출국 10위 지역을
  // 사람들이 실제로 부르는 이름은 "발리"이지 그 안의 구체 동네(우붓·
  // 쿠타·스미냑)가 아니다. "발리" 자체는 과거 사고 지역(9월 초 실측:
  // 국내 동명 지역과 오매칭)이라 WORLD_CITIES에 그대로 올리지 않기로
  // 한 결정(위 우붓 항목 주석 참고)은 유지하고, 대신 별칭으로만
  // "우붓"(이미 검증된 좌표)으로 돌린다. "코타"도 같은 이유로
  // "코타키나발루"의 줄임말 별칭을 추가한다.
  발리: "우붓",
  코타: "코타키나발루",
};

/** REGION_ALIASES의 역방향 — 정본 이름 → 그 이름을 가리키는 별칭 목록. flatRegions가 각 지역 항목에 aliases를 붙일 때 쓴다. */
const ALIASES_BY_CANONICAL: ReadonlyMap<string, string[]> = (() => {
  const map = new Map<string, string[]>();
  for (const [alias, canonical] of Object.entries(REGION_ALIASES)) {
    map.set(canonical, [...(map.get(canonical) ?? []), alias]);
  }
  return map;
})();
...
// 용도라 절대값보다 상대적 순서가 중요하다). 숫자가 작을수록 인기가
// 높다. 여기 없는 국가는 undefined로 둔다 — 이 지시서가 다루지 않은
// 국가까지 순위를 지어내지 않는다(아프리카·남미 등).
const OVERSEAS_COUNTRY_POPULARITY: Readonly<Record<string, number>> = {
  일본: 1,
  베트남: 2,
  태국: 3,
  중국: 4,
...
/**
 * course-brief가 UnsupportedRegionError를 던졌을 때 AutoPipeline에게
 * "대신 이건 어떠세요"를 바로 줄 수 있게 하는 고정 추천 목록 —
 * 작업지시서 §5. 질의어(query)에 맞춰 똑똑하게 고르는 대신, 이미
 * 실측으로 검증된(경쟁률 낮고 라이브 생성이 안정적으로 동작하는)
 * 인기 해외 지역 몇 곳을 항상 같은 순서로 돌려준다 — 절반만 맞는
 * "그럴듯한 추천"보다 "확실히 되는 대안"이 지금은 더 유용하다는
 * 판단이다. query 파라미터를 지금 안 쓰는 이유도 여기 있다 — 나중에
 * 지역·테마 taxonomy가 생기면 그때 문맥 기반으로 좁히면 된다.
 */
