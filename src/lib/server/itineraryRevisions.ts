 ...
 */
export async function snapshotItineraryRevision(itineraryId: number, title: string, region: string, placesData: unknown): Promise<void> {
  await pool.query(
    `insert into itinerary_revisions ("itineraryId", title, region, "placesData") values ($1, $2, $3, $4)`,
    [itineraryId, title, region, JSON.stringify(placesData)],
  );
  await pool.query(
