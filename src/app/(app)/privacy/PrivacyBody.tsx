 ...
 *
 * page.tsx와 별도 파일로 둔 이유(2026-08-24, "아고다 반려 진단 + 제휴 심사
 * 공통 요건" 4항 작업 중 발견) — LegalDocSheet.tsx(클라이언트 컴포넌트)가
 * 이 컴포넌트를 가져다 쓰는데, page.tsx에 `export const metadata`(서버
 * 전용)를 같이 두면 Next가 그 모듈 전체를 클라이언트 번들 경로로도
 * 끌고 가려다 빌드 에러를 낸다("attempting to export metadata from a
 * component marked with use client"). 본문을 여기로 분리해 page.tsx는
 * 서버 전용(메타데이터 + 레이아웃)으로만 남긴다.
 */
